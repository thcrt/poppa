...
def sanitise_place(data: str) -> Optional[str]:
    unsure = "?" in data
    ...
    raise errors.UnknownPlaceNameError(data)


def load_data(filename: str) -> tuple[dict[str, dict[str, Any]], list[dict[str, Any]]]:
    people = {}
    ...
    for entry in batched(data, 2):
        ...
        person_id = str(entry[0][0]).strip("# .")

        person["id"] = person_id
        person["first"] = str(entry[1][1]).title()
        person["last"] = str(entry[0][1]).title()

        person["parent1"] = str(entry[0][3]).strip()
        person["parent2"] = str(entry[1][3]).strip()

        try:
            person["birth_date"] = str(entry[0][2])
            person["birth_place"] = sanitise_place(str(entry[1][2]))
            person["death_date"] = str(entry[0][6])
            person["death_place"] = sanitise_place(str(entry[1][6]))
        except errors.UnknownPlaceNameError:
            errors.show_error(
                "Unknown place name", f"While handling birth and death of #{person_id}"
            )

        person["spouse"] = str(entry[0][4]).strip()

        # Extract date and place of marriage from the same field
        person["married_date"] = None
        person["married_place"] = None
        marriage = str(entry[1][4]).strip(". ?")
        try:
            if marriage:
            ...
            errors.show_error("Unknown place name", f"While handling marriage of #{person_id}")

        # Children are present in upper and lower rows
        person["children"] = str(entry[0][5]) + " " + str(entry[1][5])

        person["notes"] = " ".join(cell for cell in (entry[0][8:] + entry[1][8:]) if cell)
